...
class Ennova_Library {
     ...
     * @since    1.0.0
     * @access   protected
     * @var      Ennova_Library_Loader    $loader    Maintains and registers all hooks for the plugin.
     */
    protected $loader;
    ...
    public function __construct() {
        ...
        $this->plugin_name = 'ennova-library';
     ...
     *
     * - Ennova_Library_Loader. Orchestrates the hooks of the plugin.
     * - Ennova_Library_i18n. Defines internationalization functionality.
     * - Ennova_Library_Admin. Defines all hooks for the admin area.
     * - Ennova_Library_Public. Defines all hooks for the public side of the site.
     *
     ...
     *
     * Uses the Ennova_Library_i18n class in order to set the domain and to register the hook
     * with WordPress.
     *
     * @since    1.0.0
     * @access   private
    ...
    public function install_demo($theme_id) {
        ...
        file_put_contents($upload['basedir'] . '/ennova_library_data/widgets.wie', fopen($theme_data_api['widget_file_url'], 'r'));
        file_put_contents($upload['basedir'] . '/ennova_library_data/customizer.dat', fopen($theme_data_api['customizer_file_url'], 'r'));
        file_put_contents($upload['basedir'] . '/ennova_library_data/data.xml', fopen($theme_data_api['data_file_url'], 'r'));


        $wiget_file = $upload['basedir'] . '/ennova_library_data/widgets.wie';
        $customizer_file = $upload['basedir'] . '/ennova_library_data/customizer.dat';
        $data_file = $upload['basedir'] . '/ennova_library_data/data.xml';
        // File exists?
        if (!file_exists($wiget_file)) {
            wp_die(esc_html__('Import file could not be found for widget. Please try again.','ennova-library'));
        }
        // File exists?
        if (!file_exists($data_file)) {
            wp_die(esc_html__('Data import file could not be found for data. Please try again.','ennova-library'));
        }
        // File exists?
        if (!file_exists($customizer_file)) {
            wp_die(esc_html__('Customizer file could not be found for data. Please try again.','ennova-library'));
        }
    ...
    public function wie_import_data($file) {
        ...
        if (empty($data) || !is_object($data)) {
            ...
            wp_die(
                    esc_html__('Import data could not be read. Please try a different file.', 'ennova-library'), '', array(
                'back_link' => true,
                    )
            );
        }
        ...
        foreach ($data as $sidebar_id => $widgets) {
            ...
            if (isset($wp_registered_sidebars[$sidebar_id])) {
                ...
                $sidebar_message = esc_html__('Widget area does not exist in theme (using Inactive)', 'ennova-library');
            }
            ...
            foreach ($widgets as $widget_instance_id => $widget) {
                ...
                if (!$fail && !isset($available_widgets[$id_base])) {
                    ...
                    $widget_message = esc_html__('Site does not support widget', 'ennova-library'); // Explain why widget not imported.
                }
                ...
                if (!$fail && isset($widget_instances[$id_base])) {
                    ...
                    foreach ($single_widget_instances as $check_id => $check_widget) {
                        ...
                        if (in_array("$id_base-$check_id", $sidebar_widgets, true) && (array) $widget === $check_widget) {
                            ...
                            $widget_message = esc_html__('Widget already exists', 'ennova-library');
                ...
                if (!$fail) {
                    ...
                    if ($sidebar_available) {
                        ...
                        $widget_message = esc_html__('Imported', 'ennova-library');
                    } else {
                        $widget_message_type = 'warning';
                        $widget_message = esc_html__('Imported to Inactive', 'ennova-library');
                    }
                }
