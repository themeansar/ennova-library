...
//		}
    ...
    function get_authors_from_import($import_data) {
        if (!empty($import_data['authors'])) {
            ...
            foreach ($import_data['posts'] as $post) {
                ...
                if (empty($login)) {
                    printf(__('Failed to import author %s. Their posts will be attributed to the current user.', 'ennova-library'), esc_html($post['post_author']));
                    echo '<br />';
                    continue;
                }
    ...
    function author_select($n, $author) {
        ...
        echo ' ' . wp_dropdown_users(array(
            ...
            'multi' => true,
            'show_option_all' => __('- Select -', 'ennova-library'),
        ...
        foreach ($this->posts as $post) {
            ...
            if ($post_exists && get_post_type($post_exists) == $post['post_type']) {
                ...
                if (is_wp_error($post_id)) {
                    printf(__('Failed to import %s &#8220;%s&#8221;', 'ennova-library'), $post_type_object->labels->singular_name, esc_html($post['post_title']));
                    if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
    ...
    function fetch_remote_file($url, $post) {
        ...
        if ((!$type || !$ext ) && !current_user_can('unfiltered_upload')) {
            return new WP_Error('import_file_error', __('Sorry, this file type is not permitted for security reasons.', 'ennova-library'));
        }
        ...
        if (!$move_new_file) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('The uploaded file could not be moved', 'ennova-library'));
        }
